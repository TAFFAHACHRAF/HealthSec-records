...
class DiseasePrediction:
    ...
    def _feature_correlation(self, data_frame=None, show_fig=False):
        if data_frame is None:
            raise ValueError("No data frame provided for feature correlation analysis.")
        
        # Select only numeric columns
        numeric_columns = data_frame.select_dtypes(include=[np.number]).columns.tolist()
        if not numeric_columns:
            raise ValueError("No numeric columns found in the provided data frame.")
        
        data_frame_numeric = data_frame[numeric_columns]
        
        # Get Feature Correlation
        corr = data_frame_numeric.corr()
        ...
        # Save or show the plot based on show_fig
        if show_fig:
            plt.show()
        else:
